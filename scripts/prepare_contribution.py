...
def prepare_contribution(
    ...
    *,
    ...
    resolved_submission_id = str(
        submission_id
        or candidate_template.get("metadata", {}).get("submission_id")
        or ""
    ).strip()
    if not SUBMISSION_ID_PATTERN.fullmatch(resolved_submission_id):
    ...
    metadata["submission_id"] = resolved_submission_id
