...
TEMPLATE_NAME="sports-live-scores"
...
REMOTE_COMMIT="$(gh api \
  "repos/${SOURCE_REPOSITORY}/commits/${SOURCE_BRANCH}" \
  --jq '.sha')"
...
"$TEMPLATE_PYTHON" "$PROJECT_ROOT/scripts/prepare_sports_contribution.py" \
...
git -c credential.helper='!gh auth git-credential' clone \
  --quiet \
  "https://github.com/${INDEX_REPOSITORY}.git" \
  "$INDEX_ROOT"
...
"$INDEX_VENV/bin/python" -m pip install \
  --quiet \
...
- Publisher/template/version: \`weynear/${TEMPLATE_NAME}@${TEMPLATE_VERSION}\`
- Source repository and full commit SHA: \`https://github.com/${SOURCE_REPOSITORY}@${SOURCE_COMMIT}\`
- Artifact: source-only preview; the trusted central builder supplies the digest
- Data provider: API-Football
- Configuration: numeric API-Football league ID, resolved to a reviewed fixtures endpoint
- Destination and audience bindings: custom score posts and paired subscriber alerts

## Checklist

- [x] Adds a new immutable semantic version.
- [x] Pins a full source commit rather than a branch or tag.
- [x] Contains no credentials, recipient IDs, bot IDs, or application IDs.
- [x] Declares minimal provider, audience, and messaging capabilities.
- [x] Passes the upstream catalog and adversarial test suites.
EOF
)"
